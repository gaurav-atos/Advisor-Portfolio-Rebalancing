...
**Version:** 1.5 | **Status:** Approved | **Date:** 22 May 2026 | **Classification:** Confidential
...
|---|---|---|---|
| FR-001 | Dashboard: client name, risk profile, drift %, last review date, proposal status | Loads ≤3s; all fields visible | Source Docs |
| FR-002 | Colour-coded drift: Green ≤5%, Amber 5–10%, Red >10% | Colours match thresholds; legend visible | Source Docs |
| FR-003 | Sort and filter by name, risk profile, drift band, date range | All combinations; updates <1s | Source Docs |
...
| FR | Requirement | AC | Source |
|---|---|---|---|
| FR-006 | Holdings: asset class, ticker, quantity, value, current %, target % | All 7 fields; loads ≤3s | Source Docs |
...
| FR-028 ★ | In-app and email to Compliance on submission | In-app ≤30s; email ≤5min | AI-Added |
...
| NFR-005 ★ | Scalability | 500 concurrent users without degradation | AI-Added |
...
| NFR-010 ★ | Responsiveness | Functional at 768px viewport | AI-Added |
| NFR-011 ★ | Data retention | 7 years audit; 5 years proposals | AI-Added |
| NFR-012 | Role access | HTTP 403 on cross-role; denials logged | Source Docs |
| NFR-013 ★ | Localisation | UTC and local timezone on all timestamps | AI-Added |
| NFR-014 ★ | Error handling | No raw stack traces shown to non-Admin users | AI-Added |
...
| Suitability Report | report_id, proposal_id, overall_result, violations (JSON) |
...
| FR Range | Epic | JIRA User Story | Screen |
|---|---|---|---|
...
|---|---|---|---|---|
| R-01 | Mock data insufficient for edge cases | Medium | High | Representative mock data set with compliance review |
| R-02 | Rules engine misses regulatory nuances | High | High | Compliance sign-off on rule set before go-live |
| R-03 | AI summary quality inconsistent | Medium | Medium | Prompt engineering standards; track edit rate |
| R-04 ★ | SSO IdP integration delays | Medium | High | Local auth fallback for non-production environments |
| R-05 ★ | Scope creep: real-time market data requests | High | Medium | Firm out-of-scope list; formal change control process |
...
| 1.4 | 22 May 2026 | Re-push via /push_brd; version from conversation history |
| 1.5 | 22 May 2026 | Re-push via /push_artifacts command following CLAUDE.md rules |
